    ...
    @Override public void onStart() {
        ...
        try {
            ...
            if (phase.equals("restart")) {
                ...
                for (String name : new String[]{"Documents", "Audios", "3D objects"}) {
                    check(!visible.contains(name), "Files must not provide default folders");
                    Uri directory = DocumentsContract.createDocument(resolver, rootUri, Document.MIME_TYPE_DIR, name);
                    try (Cursor rows = resolver.query(directory, null, null, null, null)) {
                        check(rows.moveToFirst(), "User folder must be readable");
                        int flags = rows.getInt(rows.getColumnIndexOrThrow(Document.COLUMN_FLAGS));
                        check((flags & (Document.FLAG_SUPPORTS_DELETE | Document.FLAG_SUPPORTS_RENAME | Document.FLAG_SUPPORTS_MOVE))
                            == (Document.FLAG_SUPPORTS_DELETE | Document.FLAG_SUPPORTS_RENAME | Document.FLAG_SUPPORTS_MOVE), "User folder mutations must be supported");
                    }
                    Uri renamed = DocumentsContract.renameDocument(resolver, directory, name + " renamed");
                    DocumentsContract.deleteDocument(resolver, renamed);
                    check(!children(resolver, root).contains(name), "Deleted folders must not return");
                }
